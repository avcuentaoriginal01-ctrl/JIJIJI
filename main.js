...
function resize() {
  // Limitar dpr a 2 en móvil para no sobrecargar GPU
  ...
  bgGrad = null; // invalidar caché del gradiente
}
resize();
// Debounce resize para no recalcular en cada pixel
...
// ---- Audio — autoplay con truco muted→unmuted ----
...
bgAudio.play().then(() => {
  ...
  // Si falla igual, esperar interacción
  function startAudio() {
    bgAudio.muted = false;
    bgAudio.play().catch(() => {});
